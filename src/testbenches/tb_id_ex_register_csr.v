...
//
// And the Phase 2 A-extension additions: is_amo passing through on a clean
// cycle and zeroed on flush (same group as is_muldiv - it triggers a real
// MEM-stage side effect once consumed), and amo_funct5 passing through
// unflushed (same group as csr_addr - inert whenever is_amo is 0).
// ============================================================================
...
module tb_id_ex_register_csr;
    ...
    task check5;
        ...
        begin
            checks = checks + 1;
            if (actual !== expected) begin
                $display("[%0t] FAIL: expected %05b, got %05b", $time, expected, actual);
                errors = errors + 1;
            end else begin
                $display("[%0t] pass: %05b", $time, actual);
            end
        end
    endtask
    ...
    initial begin
        ...
        is_muldiv = 1'b0;
        is_amo = 1'b1; amo_funct5 = `AMO_F5_SWAP;
        @(negedge clk);
        check1(is_amo_out, 1'b1);
        check5(amo_funct5_out, `AMO_F5_SWAP);
        is_amo = 1'b0;

        // ---------------- Flush: csr_op/ecall/ebreak/mret/illegal/is_muldiv/is_amo zeroed, csr_use_imm/csr_addr/amo_funct5 pass through ----------------
        csr_op = `CSR_OP_RW; csr_use_imm = 1'b1; csr_addr = `CSR_MCAUSE; amo_funct5 = `AMO_F5_ADD;
        reg_write = 1'b1; mem_read = 1'b1; mem_write = 1'b1; branch = 1'b1; jump = 1'b1; is_jalr = 1'b1;
        ecall = 1'b1; ebreak = 1'b1; mret = 1'b1; illegal = 1'b1; is_muldiv = 1'b1; is_amo = 1'b1;
        flush = 1'b1;
        @(negedge clk);
        flush = 1'b0;
        ecall = 1'b0; ebreak = 1'b0; mret = 1'b0; illegal = 1'b0; is_muldiv = 1'b0; is_amo = 1'b0;
        ...
        check5(amo_funct5_out, `AMO_F5_ADD); // NOT zeroed - passes through like csr_addr
